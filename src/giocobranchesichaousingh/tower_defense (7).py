...
sfondo_img = pygame.transform.scale(sfondo_img, (924, ALTEZZA_SCHERMO))
...
def calcola_punteggio(stato):
    """
    Punteggio = punti base (100 per nemico ucciso)
              + 500 * ondate completate
              + 200 * vite rimaste
              + 2000 bonus vittoria
    """
    bonus_ondate   = stato['ondata'] * 500
    bonus_vite     = stato['vite'] * 200
    bonus_vittoria = 2000 if stato['vittoria'] else 0
    return stato['punti'] + bonus_ondate + bonus_vite + bonus_vittoria
        ...
        if ha_colpito:
            ...
            if bersaglio and bersaglio['salute'] > 0:
                ...
                if e_morto:
                    ...
                    stato['punti'] += 100  # 100 punti per nemico ucciso
...
def disegna_pannello_ui(schermo, soldi, vite, ondata, punti):
    pygame.draw.rect(schermo, GRIGIO, (924, 0, 220, ALTEZZA_SCHERMO))
    font_grande  = pygame.font.Font(None, 36)
    font_piccolo = pygame.font.Font(None, 26)
    ...
    testo_punti  = font_grande.render(f"Punti: {punti}", True, (255, 165, 0))
    # Etichette
    lbl_punti    = font_piccolo.render("(x nemici + ondate + vite)", True, (180,180,180))
    schermo.blit(testo_soldi,  (934, 10))
    schermo.blit(testo_vite,   (934, 50))
    schermo.blit(testo_ondata, (934, 90))
    schermo.blit(testo_punti,  (934, 130))
    schermo.blit(lbl_punti,    (934, 162))
...
def disegna_pulsante_torre(schermo, tipo_torre, costo, pos_y, soldi, selezionato):
    ...
    rettangolo = pygame.Rect(934, pos_y, 190, 70)
...
def disegna_pulsante_ondata(schermo, ondata_in_corso):
    ...
    rettangolo = pygame.Rect(934, 460, 190, 70)
...
def disegna_game_over(schermo, vittoria, punteggio, classifica):
    ...
    font_grande  = pygame.font.Font(None, 46)
    font_medio   = pygame.font.Font(None, 34)
    font_piccolo = pygame.font.Font(None, 28)

    cx = LARGHEZZA_SCHERMO // 2

    if vittoria:
        testo_principale = font_titolo.render("VITTORIA!", True, GIALLO)
    else:
        testo_principale = font_titolo.render("GAME OVER", True, ROSSO)

    schermo.blit(testo_principale, (cx - testo_principale.get_width() // 2, 80))

    # Punteggio finale
    t_score = font_grande.render(f"Punteggio: {punteggio}", True, (255, 165, 0))
    schermo.blit(t_score, (cx - t_score.get_width() // 2, 175))

    # Dettaglio formula
    t_det = font_piccolo.render("(nemici x100) + (ondate x500) + (vite x200) + (vittoria +2000)", True, (180,180,180))
    schermo.blit(t_det, (cx - t_det.get_width() // 2, 220))

    # Classifica
    pygame.draw.line(schermo, GIALLO, (cx - 180, 260), (cx + 180, 260), 2)
    t_cls = font_medio.render("MIGLIORI PUNTEGGI", True, GIALLO)
    schermo.blit(t_cls, (cx - t_cls.get_width() // 2, 270))

    colori_pos = [GIALLO, (192,192,192), (205,127,50), BIANCO, BIANCO]
    for i, score in enumerate(classifica[:5]):
        colore = colori_pos[i] if i < len(colori_pos) else BIANCO
        marker = ["🥇","🥈","🥉","4°","5°"][i] if i < 5 else f"{i+1}°"
        t = font_medio.render(f"{marker}  {score}", True, colore)
        schermo.blit(t, (cx - t.get_width() // 2, 305 + i * 34))

    pygame.draw.line(schermo, GRIGIO, (cx - 180, 480), (cx + 180, 480), 1)
    testo_riavvio = font_piccolo.render("Premi  R  per ricominciare", True, BIANCO)
    schermo.blit(testo_riavvio, (cx - testo_riavvio.get_width() // 2, 492))


def disegna_tutto(schermo, stato, pulsanti_torri, immagini, font_piccolo, classifica=[]):
    ...
    disegna_pannello_ui(schermo, stato['soldi'], stato['vite'], stato['ondata'], calcola_punteggio(stato))
    ...
    if stato['game_over']:
        disegna_game_over(schermo, stato['vittoria'], stato['punteggio_finale'], classifica)
...
def gestisci_click(stato, mouse_x, mouse_y, pulsanti_torri, pulsante_ondata):
    ...
    if mouse_x < 924 and stato['torre_selezionata']:
        griglia_x, griglia_y = pixel_a_griglia(mouse_x, mouse_y)
...
def main():
    ...
    classifica = []  # lista punteggi migliori (sessione)
    ...
    while in_esecuzione:
        ...
        # Salva in classifica appena il gioco termina (una volta sola)
        if stato['game_over'] and stato['punteggio_finale'] > 0:
            if stato['punteggio_finale'] not in classifica:
                classifica.append(stato['punteggio_finale'])
                classifica.sort(reverse=True)
                classifica = classifica[:5]

        disegna_tutto(schermo, stato, pulsanti_torri, immagini, font_piccolo, classifica)
